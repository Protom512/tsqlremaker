...
/// DocumentAnalysisから診断を生成する（キャッシュ利用・#132: `config` 駆動）。
///
/// `config.select_star_severity` が `SELECT *` 警告の重大度を決定する。
/// パーサーエラーの重大度は設定非対象（常に ERROR/WARNING はパーサー由来）。
#[must_use]
pub fn diagnose(analysis: &DocumentAnalysis, config: &DiagnosticsConfig) -> Vec<Diagnostic> {
    ...
    let star_severity = config.select_star_severity.into();
    diags.extend(semantic_diagnostics(analysis, star_severity));
    diags
}
...
fn semantic_diagnostics(
    ...
    for stmt in &analysis.statements {
        collect_select_star_warnings(stmt, analysis, &mut diags, severity);
    }
    diags
}
...
fn collect_select_star_warnings(
    ...
    match stmt {
        Statement::Select(sel) => {
            for item in &sel.columns {
                if let SelectItem::Wildcard = item {
                    if let Some(diag) = make_star_diagnostic(analysis, &sel.span, severity) {
                        diags.push(diag);
                    }
                }
            }
            ...
            if let Some(from_clause) = &sel.from {
                for table_ref in &from_clause.tables {
                    collect_from_table_ref(table_ref, analysis, diags, severity);
                }
            }
        }
        Statement::Insert(insert) => {
            if let tsql_parser::ast::InsertSource::Select(sub_sel) = &insert.source {
                collect_select_star_warnings(
                    &Statement::Select(sub_sel.clone()),
                    analysis,
                    diags,
                    severity,
                );
        ...
        Statement::Update(update) => {
            collect_from_table_ref(&update.table, analysis, diags, severity);
        ...
        Statement::If(if_stmt) => {
            collect_select_star_warnings(&if_stmt.then_branch, analysis, diags, severity);
            if let Some(else_branch) = &if_stmt.else_branch {
                collect_select_star_warnings(else_branch, analysis, diags, severity);
            }
        }
        Statement::While(while_stmt) => {
            collect_select_star_warnings(&while_stmt.body, analysis, diags, severity);
        }
        Statement::Block(block) => {
            for child in &block.statements {
                collect_select_star_warnings(child, analysis, diags, severity);
            }
        }
...
fn collect_from_table_ref(
    ...
    match table_ref {
        ...
        tsql_parser::ast::TableReference::Joined { joins, .. } => {
            for join in joins {
                collect_from_table_ref(&join.table, analysis, diags, severity);
            }
        }
...
/// SELECT * に対応するDiagnosticを生成する（重大度は `severity`・#132 で可変）。
...
mod tests {
    ...
    fn test_valid_sql_no_parse_errors() {
        ...
        let diags = diagnose(&analysis, &DiagnosticsConfig::default());
    ...
    // === configuration-driven severity (#132) ===

    #[test]
    fn config_select_star_severity_hint_overrides_default_warning() {
        let analysis = crate::analysis::DocumentAnalysis::new("SELECT * FROM users");
        let cfg = DiagnosticsConfig {
            select_star_severity: Severity::Hint,
        };
        let diags = diagnose(&analysis, &cfg);
        let star = diags.iter().find(|d| d.message.contains("SELECT *"));
        assert!(star.is_some(), "SELECT * should still be reported");
        assert_eq!(
            star.unwrap().severity,
            Some(DiagnosticSeverity::HINT),
            "configured Hint must override the default Warning"
        );
    }

    #[test]
    fn config_select_star_severity_error() {
        let analysis = crate::analysis::DocumentAnalysis::new("SELECT * FROM users");
        let cfg = DiagnosticsConfig {
            select_star_severity: Severity::Error,
        };
        let diags = diagnose(&analysis, &cfg);
        let star = diags.iter().find(|d| d.message.contains("SELECT *"));
        assert_eq!(star.unwrap().severity, Some(DiagnosticSeverity::ERROR));
    }

    #[test]
    fn config_default_severity_matches_legacy_warning() {
        // Default DiagnosticsConfig reproduces the pre-#132 Warning behaviour.
        let analysis = crate::analysis::DocumentAnalysis::new("SELECT * FROM users");
        let diags = diagnose(&analysis, &DiagnosticsConfig::default());
        let star = diags.iter().find(|d| d.message.contains("SELECT *"));
        assert_eq!(star.unwrap().severity, Some(DiagnosticSeverity::WARNING));
    }
